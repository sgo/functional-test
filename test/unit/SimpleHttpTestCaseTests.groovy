...
class SimpleHttpTestCaseTests extends GroovyTestCase {
    ...
    void doURLTests(base) {
        ...
        def p = [:]
        assertEquals base + 'index.gsp', tester.makeRequestURL(p, 'index.gsp').toString()
        assertEquals base + 'index.gsp', tester.makeRequestURL(p, '/index.gsp').toString()
        assertEquals base + 'controller/action',
                tester.makeRequestURL(p, '/controller/action').toString()
        tester.baseURL = base + 'controller/action'
        assertEquals base + 'controller/something',
                tester.makeRequestURL(p, 'something').toString()

        tester.baseURL = base
        p.url = base + 'login.gsp'
        assertEquals base + 'admin/account/list', tester.makeRequestURL(p, 'admin/account/list').toString()
    }
    ...
    /*	
     ...
     void testFindingForms() {
         ...
         mockPage.demand.getHtmlElementById(1) { id ->
